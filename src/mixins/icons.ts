...
import { type MaybeNumberish, type TSizeOptionProps, useRenderSVG } from 'vue-mdbootstrap';

declare type TCustomIconProps = TSizeOptionProps & {
  class?: string | string[];
};

export function iconComments(props: TCustomIconProps): VNode {
  ...
  return useRenderSVG(
    svg,
    (props.size as MaybeNumberish) ?? props.height ?? 24,
    (props.size as MaybeNumberish) ?? props.width ?? 24,
    props.class
  );
}

export function iconListCheck(props: TCustomIconProps): VNode {
  // Font Awesome Free 6.4.0 by @fontawesome - https://fontawesome.com/
  const svg =
